...
@section('content')
    <div class="max-w-md bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('admin.payment-methods.update', $paymentMethod) }}" class="space-y-4">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nama Metode</label>
                <input type="text" name="name" value="{{ old('name', $paymentMethod->name) }}" required autofocus
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.payment-methods.index') }}" class="text-sm text-gray-500 px-4 py-2">Batal</a>
            </div>
        </form>
    </div>
@endsection
